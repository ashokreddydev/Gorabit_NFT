...
contract Auction is Context, Ownable, ReentrancyGuard, ERC1155Holder, INFTSale {
    ...
    event Bid(uint256 lotID, uint256 nftID,uint256 currentBid,address currentBidder,bool firstBid, uint256 bidTime);
    ...
    function stake(
        ...
        totalLots++;
        lots[totalLots] = AuctionLot(startTime, endTime, nftID, bidIncrementPercent, artist, price, true,false);
    ...
    function bid(uint256 lotID) payable public nonReentrant {
        require(_msgSender() != contractOwner,"Owner cannot Bid" );
        ...
        lot.firstBid = false;
        lots[lotID] = lot;
        emit Bid(lotID,lot.nftID,lot.currentBid,lot.currentBidder,lot.firstBid,block.timestamp);
    ...
    function payout(uint256 lotID) payable public nonReentrant {
        AuctionLot memory lot = lots[lotID];
        require(_msgSender() == contractOwner, "Invalid owner");
...
contract NFT_FM is ERC1155, Ownable, ReentrancyGuard {
    ...
    event MintAndStake(
      ...
      function addTokenData(string memory ipfsId,uint256 tokenID) public {
        // In a mapping, all elements are defined. They're just empty by default
        // So, just setting the id will create a new user;
        // tokenDatas[tokenID].imageUrl = imageUrl;
        tokenDatas[tokenID].ipfsId = ipfsId;
        tokenDatas[tokenID].tokenID = tokenID;
      }
      function getTokendata (uint256 tokenID)  public view returns (string memory ipfsId)
      { 
          //imageUrl = tokenDatas[tokenID].imageUrl;
          ipfsId = tokenDatas[tokenID].ipfsId;
          
         // return (tokenDatas[tokenID].imageUrl,tokenDatas[tokenID].tokenName);
      }
    ...
    function setAuction (
       ...
       bytes memory data = abi.encode(endTime,bidIncrementPercent);
        // mintAndStake(
        // quantity,
        // price,
        // startTime,
    ...
    /*
        ...
        */
        //tokenDatas[nftID+1].imageUrl = imageUrl;
        tokenDatas[nftID+1].ipfsId = ipfsId;
        tokenDatas[nftID+1].tokenID = nftID;
        nftID++;
    ...
    /*
        ...
        */
        //tokenDatas[nftID+1].imageUrl = imageUrl;
        bytes memory data = abi.encode(feePercent);
        tokenDatas[nftID+1].ipfsId= ipfsId;
        tokenDatas[nftID+1].tokenID = nftID;
